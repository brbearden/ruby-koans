...
class Proxy
  ...
  def channel=(channel)
    @messages << :channel=
    @called[:channel=] = @called[:channel=] + 1
    @object.send(:channel=, channel)
  end
  ...
  def power
    @messages << :power
    @called[:power] = @called[:power] + 1
    @object.send(:power)
  end

  def on?
    @messages << :on?
    @called[:on?] = @called[:on?] + 1
    @object.send(:on?)
  end
  ...
  def method_missing(meth_id, *args)
    @messages << meth_id
    @called[meth_id] = @called[meth_id] + 1
    @object.send(meth_id, *args)
  end
